...
def getPrompt(intent, query):

    prompt = intentToPrompt[intent]
    prompt += f"""
...
def response(intent, query):

    prompt = getPrompt(intent, query)
    response = sendPrompt(prompt)
    return response


user_query = "What was Tesla's net income last year?"
print(response("stat", user_query))

chart_query = "Show me a bar chart of Apple's quarterly revenue for 2023."
print(response("chart", chart_query))

chitchat_query = "Hi, how are you?"
print(response("chitchat", chitchat_query))